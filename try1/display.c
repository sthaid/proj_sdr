...
#define SDL_EVENT_ADJUST   (SDL_EVENT_USER_DEFINED + 0)  // for length MAX_BAND
    ...
    while (true) {
        ...
        if (total_width) {
            ...
            for (i = 0; i < max_band; i++) {
                ...
                sdl_register_event(&loc, SDL_EVENT_ADJUST+i, SDL_EVENT_TYPE_MOUSE_WHEEL);

                cumulative_width += b->width;
            }
        }
